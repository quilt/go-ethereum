...
type ChainConfig struct {
	...
	AlpineBlock         *big.Int `json:"alpineBlock,omitempty"`
...
func (c *ChainConfig) String() string {
	...
	return fmt.Sprintf("{ChainID: %v Homestead: %v DAO: %v DAOSupport: %v EIP150: %v EIP155: %v EIP158: %v Byzantium: %v Constantinople: %v Petersburg: %v Istanbul: %v, Muir Glacier: %v, Berlin: %v, YOLO v3: %v, Alpine: %v, Engine: %v}",
		c.ChainID,
		c.HomesteadBlock,
		c.DAOForkBlock,
		c.DAOForkSupport,
...
func (c *ChainConfig) IsAlpine(num *big.Int) bool {
	return isForked(c.AlpineBlock, num)
}
...
func (c *ChainConfig) checkCompatible(newcfg *ChainConfig, head *big.Int) *ConfigCompatError {
	...
	if isForkIncompatible(c.AlpineBlock, newcfg.AlpineBlock, head) {
		return newCompatError("Alpine fork block", c.AlpineBlock, newcfg.AlpineBlock)
	}
