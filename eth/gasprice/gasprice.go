...
func (gpo *Oracle) SuggestPrice(ctx context.Context, tip bool) (*big.Int, error) {
	...
	for sent < gpo.checkBlocks && number > 0 {
		go gpo.getBlockValues(ctx, tip, types.MakeSigner(gpo.backend.ChainConfig(), big.NewInt(int64(number))), number, sampleNumber, result, quit)
		sent++
		exp++
		number--
	}
...
func (gpo *Oracle) getBlockValues(ctx context.Context, tip bool, signer types.Signer, blockNum uint64, limit int, result chan results, quit chan struct{}) {
	...
	var prices []*big.Int
	for i := range txs {
		if txs[i].GasPriceIntCmp(common.Big1) <= 0 {
			continue
		}
