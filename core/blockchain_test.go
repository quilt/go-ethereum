...
func TestEIP1559Transition(t *testing.T) {
	var (
		...
		gspec   = &Genesis{
			Config: params.TokyoChainConfig,
			Alloc: GenesisAlloc{
				addr1: {Balance: funds},
				addr2: {Balance: funds},
				// The address 0xAAAA sloads 0x00 and 0x01
	...
	blocks, _ := GenerateChain(gspec.Config, genesis, engine, db, 1, func(i int, b *BlockGen) {
		...
		tx := types.NewDynamicFeeTransaction(gspec.Config.ChainID, 0, aa, big.NewInt(0), 30000, new(big.Int).Mul(big.NewInt(5), big.NewInt(params.GWei)), big.NewInt(2), nil, &accesses)
		tx, _ = types.SignTx(tx, types.NewEIP2718Signer(gspec.Config.ChainID), key1)
	...
	// 3: Ensure that miner recieved only the tx's tip.
	...
	blocks, _ = GenerateChain(gspec.Config, block, engine, db, 1, func(i int, b *BlockGen) {
		...
		tx, _ = types.SignTx(tx, types.NewEIP2718Signer(gspec.Config.ChainID), key2)
	...
	// 6+5: Ensure that miner recieved only the tx's effective tip.
	actual = state.GetBalance(block.Coinbase())
	expected = new(big.Int).Add(
