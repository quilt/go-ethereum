	...
	var (
		...
		blocks, _ = GenerateChain(params.EIP1559ChainConfig, genesis, ethash.NewFaker(), testdb, 5, func(i int, gen *BlockGen) {
			...
			case 0:
				...
				tx, _ := types.SignTx(types.NewTransaction(gen.TxNonce(addr1), addr2, big.NewInt(10000), params.TxGas, new(big.Int), nil), signer, key1)
				gen.AddTx(tx)
			case 1:
				// In block 2, addr1 sends some more ether to addr2.
				// addr2 attempts to pass it on to addr3 using a EIP1559 transaction
				tx1, _ := types.SignTx(types.NewTransaction(gen.TxNonce(addr1), addr2, big.NewInt(1000), params.TxGas, new(big.Int), nil), signer, key1)
				tx2, _ := types.SignTx(types.NewEIP1559Transaction(gen.TxNonce(addr2), addr3, big.NewInt(1000), params.TxGas, nil, nil, new(big.Int), new(big.Int).SetUint64(params.EIP1559InitialBaseFee)), signer, key2)
				gen.AddTx(tx1)
				gen.AddTx(tx2)
			case 2:
...
func TestHeaderVerificationEIP1559Finalized(t *testing.T) {
	...
	var (
		...
		blocks, _ = GenerateChain(params.EIP1559FinalizedChainConfig, genesis, ethash.NewFaker(), testdb, 5, func(i int, gen *BlockGen) {
			...
			case 0:
				// In block 1, addr1 sends addr2 some ether.
				tx, _ := types.SignTx(types.NewEIP1559Transaction(gen.TxNonce(addr1), addr2, big.NewInt(10000), params.TxGas, nil, nil, new(big.Int), new(big.Int).SetUint64(params.EIP1559InitialBaseFee)), signer, key1)
				gen.AddTx(tx)
			case 1:
				// In block 2, addr1 sends some more ether to addr2.
				// addr2 attempts to pass it on to addr3 using a EIP1559 transaction
				tx1, _ := types.SignTx(types.NewEIP1559Transaction(gen.TxNonce(addr1), addr2, big.NewInt(1000), params.TxGas, nil, nil, new(big.Int), new(big.Int).SetUint64(params.EIP1559InitialBaseFee)), signer, key1)
				tx2, _ := types.SignTx(types.NewEIP1559Transaction(gen.TxNonce(addr2), addr3, big.NewInt(1000), params.TxGas, nil, nil, new(big.Int), new(big.Int).SetUint64(params.EIP1559InitialBaseFee)), signer, key2)
				gen.AddTx(tx1)
				gen.AddTx(tx2)
			case 2:
...
func TestCalcGasLimitAndBaseFee(t *testing.T) {
	var forkBlock uint64 = 110000000
	var finalizedBlock uint64 = forkBlock + params.EIP1559DecayRange
	...
	testConditions := []struct {
	...
	}{
		{
			...
			new(big.Int).SetUint64(forkBlock),
			nil,
			8000000,
			8000000,
			big.NewInt(1100000000),
			new(big.Int).SetUint64(forkBlock - 1),
		...
		// Half way between initialization and finalization we should be at a 25 : 75 legacy : eip1559 split
		{
			...
			10000000,
			new(big.Int).SetUint64(params.EIP1559InitialBaseFee),
			new(big.Int).SetUint64((forkBlock + (finalizedBlock-forkBlock)/2) - 1),
		...
		// At and beyond EIP1559 finalization the GasLimit (for the EIP1559 pool) is the entire MaxGasEIP1559
		{
			...
			new(big.Int).SetUint64(forkBlock),
			new(big.Int).SetUint64(finalizedBlock),
			8000000,
			9000000,
			big.NewInt(1086250000),
			new(big.Int).SetUint64(finalizedBlock - 1),
		...
		},
		{
			...
			new(big.Int).SetUint64(finalizedBlock + 10000),
			params.MaxGasEIP1559,
